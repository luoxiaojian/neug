...
bool parse_ldbc_timestamp_ms(const std::string& token, int64_t* out_ms) {
  return ldbc::parse_ldbc_timestamp_ms(token.data(), token.size(), out_ms);
}

bool parse_ldbc_long_date_ms(const std::string& token, int64_t* out_ms) {
  return ldbc::parse_ldbc_long_date_ms(token.data(), token.size(), out_ms);
}
...
execution::Value parse_date_value(const std::string& token) {
  int64_t millis = 0;
  if (parse_ldbc_timestamp_ms(token, &millis) ||
      parse_ldbc_long_date_ms(token, &millis)) {
    Date d;
    ...
    while (output_rows < chunk_size_ && reader_->read_row(row)) {
      ...
      for (size_t column_index = 0; column_index < builders.size();
        ...
        append_csv_field_to_builder(
            ...
            file_path_, current_row_number_, selected_column_names_[column_index]);
      }
      output_rows += 1;
    }
...
#define SET_TYPED_VALUE(enum_val, ctype)                                    \
  case DataTypeId::enum_val: {                                              \
    auto* typed = dynamic_cast<TypedColumn<ctype>*>(col.get());             \
...
#undef SET_TYPED_VALUE
    case DataTypeId::kDate: {
      auto* typed = dynamic_cast<TypedColumn<Date>*>(col.get());
      typed->set_value(vids[k], val.GetValue<Date>());
      break;
    }
    case DataTypeId::kTimestampMs: {
      auto* typed = dynamic_cast<TypedColumn<DateTime>*>(col.get());
      typed->set_value(vids[k], val.GetValue<DateTime>());
      break;
    }
    case DataTypeId::kInterval: {
      auto* typed = dynamic_cast<TypedColumn<Interval>*>(col.get());
      typed->set_value(vids[k], val.GetValue<Interval>());
      break;
    }
    case DataTypeId::kVarchar: {
      auto* typed = dynamic_cast<TypedColumn<std::string_view>*>(col.get());
