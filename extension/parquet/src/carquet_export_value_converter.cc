...
neug::Status writeDateColumn(carquet_writer_t* writer,
  ...
  for (int64_t i = 0; i < num_rows; ++i) {
    days.push_back(static_cast<int32_t>(values.Get(static_cast<int>(i)) / 86400000LL));
  }
...
neug::Status writeListColumn(carquet_writer_t* writer,
  ...
  if (elements.has_string_array()) {
    ...
    const auto& element_values = elements.string_array().values();
    const auto& element_validity = elements.string_array().validity();
    ...
    for (int64_t row = 0; row < num_rows; ++row) {
      ...
      for (int32_t idx = start; idx < end; ++idx) {
        ...
        storage.push_back(element_values.Get(idx));
      }
    }
  ...
  if (elements.has_int32_array()) {
    ...
    std::vector<int16_t> def_levels;
    std::vector<int16_t> rep_levels;
    const auto& element_values = elements.int32_array().values();
    const auto& element_validity = elements.int32_array().validity();
    const auto& list_validity = list_arr.validity();
  ...
  if (elements.has_int64_array()) {
    ...
    std::vector<int16_t> def_levels;
    std::vector<int16_t> rep_levels;
    const auto& element_values = elements.int64_array().values();
    const auto& element_validity = elements.int64_array().validity();
    const auto& list_validity = list_arr.validity();
