...
std::shared_ptr<arrow::Array> protoArrayToArrowArray(
  ...
  if (proto_array.has_date_array()) {
    // date_array stores millisecond values; convert to days (Date32) using
    // floor division so negative timestamps map correctly.
    auto& arr = proto_array.date_array();
    arrow::Date32Builder builder(pool);
    for (int i = 0; i < arr.values_size(); ++i) {
      if (writer::StringFormatBuffer::validateProtoValue(arr.validity(), i)) {
        int64_t ms = arr.values(i);
        int32_t day = static_cast<int32_t>(ms / 86400000LL);
        if (ms < 0 && ms % 86400000LL != 0) {
          --day;
        }
        auto status = builder.Append(day);
        if (!status.ok()) {
