...
size_t headerCallback(char* buffer, size_t size, size_t nitems, void* userdata) {
  ...
  if (header.rfind(kPrefix, 0) == 0) {
    try {
      *content_length = std::stoll(header.substr(strlen(kPrefix)));
    } catch (...) {
    }
  }
  return total;
...
int64_t S3Client::readObjectRange(const std::string& bucket,
  ...
  auto response = performRequest(config_, "GET", std::move(parts), range_header);
  ...
  if (static_cast<int64_t>(response.body.size()) > length) {
    THROW_IO_EXCEPTION("S3 GET returned more bytes than requested");
  }
  if (!response.body.empty()) {
    std::memcpy(out, response.body.data(), response.body.size());
  }
